...
const goodLines = lines.slice(0, 93);
const newEnd = `              {filteredProducts.map((item, idx) => (
                <motion.div 
                  key={idx} 
...
fs.writeFileSync('src/pages/Collections.tsx', goodLines.join('\n') + '\n' + newEnd);
console.log("Fixed Collections.tsx");
